...
const RequestList = () => {
    const [dataGruas, setDataGruas] = useState<Grua[]>([])
    ...
    const fetchData = async () => {
        try {
            const GruasQuery = query(collection(db, 'Gruas'))
            const snapshot = await getDocs(GruasQuery)
            const Gruas = snapshot.docs.map(
                (docSnap) => ({ ...docSnap.data(), id: docSnap.id }) as Grua,
            )
            setDataGruas(Gruas)
        } catch (error) {
            console.error('Error al cargar Gruas:', error)
        }
    }
    ...
    const openGruaDetails = (Grua: Grua) => {
    ...
    const handleExportToExcel = () => {
        ...
        const tableData = rowsToExport.map((row) => {
            ...
            return {
                [encabezados.nombre_Grua]: r.nombre_Grua ?? '',
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, 'Grúas')
        XLSX.writeFile(workbook, 'gruas.xlsx')
        toast.push(
    ...
    const columns: ColumnDef<Grua>[] = [
        {
            header: 'Imagen',
            accessorKey: 'Grua_image',
            cell: ({ row }) => {
                const Grua = row.original
                const images = Grua.Grua_image
                if (images && images.length > 0) {
                    return (
                        <div className="flex items-center">
                            <img
                                src={images[0]}
                                alt={Grua.nombre_Grua}
        ...
        {
            header: 'Nombre de la Grua',
            accessorKey: 'nombre_Grua',
            cell: ({ row }) => (
                <div className="font-medium text-gray-900">
                    {row.original.nombre_Grua}
                </div>
            ),
        },
        {
            ...
            cell: ({ row }) => (
                <div className="flex gap-2">
                    <Button
                        size="sm"
                        variant="default"
                        onClick={() => openGruaDetails(row.original)}
    ...
    const table = useReactTable({
        ...
        globalFilterFn: (row, _columnId, filterValue) => {
            ...
            if (!term) return true
            const r = row.original
            const nombre = (r.nombre_Grua ?? '').toLowerCase()
    ...
    return (
        <>
            ...
            {/* Modal para mostrar detalles de la Grua */}
            <Dialog
                isOpen={modalIsOpen}
                ...
                {selectedGrua && (
                    <div>
                        ...
                        <div className="space-y-6">
                            {selectedGrua.Grua_image && selectedGrua.Grua_image.length > 0 && (
                                <div>
                                    <h3 className="text-lg font-semibold mb-3">Imágenes de la Grua</h3>
                                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                        {selectedGrua.Grua_image.map((image, index) => (
                                            <img
                                                key={index}
                                                src={image}
                                                alt={`Imagen ${index + 1}`}
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                <div className="bg-gray-50 p-4 rounded-lg">
                                    ...
                                    <div className="space-y-3">
                                        <div>
                                            <span className="font-medium text-gray-700">Nombre de la Grua:</span>
                                            <p className="text-gray-900 font-semibold">{selectedGrua.nombre_Grua}</p>
                                        </div>
                                        <div>
                                        ...
                                        <div>
                                            ...
                                            <p className="text-gray-900">
                                                {selectedGrua.precio != null
                                                    ? `$${Number(selectedGrua.precio).toLocaleString()}`
                                                    : '—'}
                                            </p>
                                        </div>
                                        <div>
                                            ...
                                            <div className="mt-1">
                                                {getStatusIcon(selectedGrua.estatus ?? false)}
                                            </div>
                                        </div>
                                    </div>
                                </div>
